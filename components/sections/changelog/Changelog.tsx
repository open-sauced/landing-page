...
const Changelog: FC<ChangelogProps> = ({
  changelog: { title, date, changelogCategory, changelogContent, topics},
  index,
  count
}) => {
  ...
  return (
    <div className="flex gap-x-10">
      <div style={index+1 === count ? {} : borderStyle} className={`hidden  tablet:flex relative pb-28 flex-1 border-textPrimary border-opacity-50 pl-10 max-w-md flex-col`}>
        <span>
      ...
      <div  className="flex-1 relative pb-10 border-l-2 tablet:border-0 tablet:pl-0 pl-6 ">
        ...
        <div style={containerHeightStyle}>
          <div ref={contentRef}>
            <ReactMarkdown
              className="
                prose prose-sm
                prose-headings:text-textPrimary prose-p:text-textPrimary prose-p:text-base
              ">
              { changelogContent }
            </ReactMarkdown>
          </div>

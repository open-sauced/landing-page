...
const Logos = ({ users }: LogosProps) => {
  return (
    <section >
      ...
      <div className=" border-[1px] border-gray300 mobile:rounded-2xl tablet:rounded-full grid grid-cols-2 sm:grid-cols-3 tablet:grid-cols-3 laptop:grid-cols-4 gap-x-8 tablet:gap-x-28 gap-y-16 tablet:gap-y-11 mb-24 py-8 tablet:py-16 items-center justify-items-center grid-rows-[38px] auto-rows-[38px] px-4 tablet:px-20">
        {users.map((user) => (
          <a
            key={user._id}
            href={user.website}

...
const Index: NextPage<BlogPageProps> = ({
  data: { seoData, homePageData, featuredBlog },
}) => {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        ...
        setLoadingBlog(true)
        await getBlogs(blogLimit).then((blogs) =>
          setBlogs(blogs as unknown as SanityBlog[])
        )
    ...
    <>
      ...
      <div className="max-w-6xl mx-auto px-8">
        <BlogBackgroundDrip>
          ...
          <h1 className="text-center font-thin tracking-[1rem] text-[4rem] py-32">
            Blog
          </h1>

          <div className="py-[15px] flex items-center gap-x-[8px]">
            <FaPizzaSlice size={25} className="text-gray-800" />
            <h6 className="font-semibold text-[25px] text-gray-800">
              Featured
            </h6>
          </div>
            ...
            {featuredBlog.map(
              ({
                ...
                <div key={_id} className=" flex-grow">
                  <Link
                    passHref
                    href={getBlogLink(isNative, slug?.current, blogUrl)}
                  >
                  ...
                  <div className="mt-[20px]">
                    <div className="flex gap-[8px] w-full flex-wrap items-center mt-[20px] tablet:mt-[50px] mb-[15px]">
                      ...
                      {topics?.slice(0, 6).map((topic, index) => (
                        <div
                          key={_id + index}
                          className="flex items-center gap-[5px] min-h-[6px] overflow-hidden"
                        >
                    ...
                    <Link
                      ...
                      passHref
                    >
                      <h6 className="font-bold text-[17px] text-gray-700 cursor-pointer">
                        {title}
                      </h6>
                    </Link>
                    ...
                    <Link
                      ...
                      passHref
                    >
                      <p className="text-[18px] text-gray-700 mt-[10px] tablet:mt-[30px] cursor-pointer">
                        {summary?.slice(0, 180)}
                      </p>
                    </Link>
                  </div>
                ...
                ({
                  ...
                  <div key={_id}>
                    <div className="flex flex-col tablet:flex-row gap-x-[20px]">
                      ...
                      <div className="flex-1 mt-[15px] tablet:mt-0">
                        <Link
                          ...
                          passHref
                        >
                          <h1 className="font-bold text-[20px] cursor-pointer text-gray-700">
                            {title}
                          </h1>
                        </Link>
                        ...
                        <div className="flex gap-[8px] items-center mt-[10px] mb-[20px]">
                          ...
                          {topics?.map((topic: string, index: number) => (
                            <div
                              key={topic + index + 1}
                              className="flex items-center gap-[5px] min-h-[6px]"
                            >
                              <div className="w-[6px] h-[6px] bg-orange-600 rounded-full"></div>
                    ...
                    <Link
                      ...
                      passHref
                    >
                      <p className="text-[18px] cursor-pointer hidden tablet:block laptop:hidden mt-[20px] mb-[30px]">
                        {summary?.slice(0, 180)}
                      </p>
                    </Link>
                  </div>
        ...
        <div className="flex justify-center my-[30px]">
          <div
            ...
            className="flex gap-x-3 justify-center items-center h-[40px] w-[150px] cursor-pointer rounded-[20px] shadow-orange-600 shadow"
          >
            <div className="w-[20px] h-[20px] rounded-full">
              <Image alt="vector art" src={ellipseOrange} />
            </div>
            <p className="text-[15px] font-semibold text-gray-600">
              {loadingBlog ? 'Loading ..' : 'Load more'}
            </p>
          </div>
        </div>
      </div>
